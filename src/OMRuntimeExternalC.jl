...
const installedLibPath = if _libpath !== nothing
    ...
    if libpath === nothing
      throw("Could not the ModelicaStandardTables library")
    end
    libpath
end
...
function callExternalTable()
  ...
  ModelicaStandardTables_CombiTable1D_init2_julia(fileName, tableName, tableCShape, size(table,1), size(table,2), columns, size(columns,1), LinearSegments, LastTwoPoints, verbose)
end
